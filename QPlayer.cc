  ...
  if (msg.sType == DUI_MSGTYPE_CLICK){
    if (msg.pSender->GetName() == _T("btnOpen")){
      ...
      if (url.GetLength()) {
        ...
        ofn.lpstrFilter = "Flv file(*.flv)\0*.flv\0";
        if (GetOpenFileNameA(&ofn)) {
          player_.start(szFile);
          //player_.OpenFile(szFile);
          //SetTimer(GetHWND(), UM_RENDER, 33, NULL);
        }
      }
      UpdateUI();
    }
...
void JiuquPlayer::OnRenderFrame(const cricket::VideoFrame* decodedImage)
{
  video_wnd_.RenderFrame(decodedImage);
}
*/
void QPlayer::OnProgress(uint32_t cur, uint32_t duration)
{
  ...
  // if (duration > 0) slide_player_->SetMaxValue(duration);
}
...
{
  video_wnd_.SetText("Rtmp连接失败", RGB(255, 0, 0));
}
